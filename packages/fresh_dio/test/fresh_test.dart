  ...
  group('Fresh', () {
    ...
    group('configure token', () {
      group('setToken', () {
        ...
        test('adds unauthenticated status when call setToken(null)', () async {
          ...
          when(() => tokenStorage.write(any()))
              .thenAnswer((_) => Future.value());
          when(() => tokenStorage.delete()).thenAnswer((_) => Future.value());
          final fresh = Fresh.oAuth2(
            tokenStorage: tokenStorage,
            refreshToken: emptyRefreshToken,
          );
    ...
    group('onRequest', () {
      ...
      test(
        ...
        when(() => tokenStorage.write(any())).thenAnswer((_) => Future.value());
        final fresh = Fresh.oAuth2(
          tokenStorage: tokenStorage,
          refreshToken: emptyRefreshToken,
        );
    ...
    group('onResponse', () {
      test('returns untouched response when token is null', () async {
        when(() => tokenStorage.read()).thenAnswer((_) => Future.value());
        when(() => tokenStorage.write(any())).thenAnswer((_) => Future.value());
